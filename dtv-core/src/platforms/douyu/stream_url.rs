...
impl DouYu {
    ...
    pub async fn get_real_url_with_quality(
        ...
        let sign_data = self
            .build_sign_params(&real_room_id)
            .await
            .map_err(|e| DtvError::api(e.to_string()))?;
        let play_info = self
            .get_play_qualities(&real_room_id, &sign_data)
            .await
            .map_err(|e| DtvError::api(e.to_string()))?;
        ...
        // 1. 先打印简要的匹配尝试
        tracing::debug!(target: "douyu_stream", "[Douyu] Matching quality '{}' -> rate {}", quality, selected_rate);

        // 2. 将厚重的原始数据放进 TRACE，或者只在满足某种条件时打印
        tracing::trace!(target: "douyu_stream", "Full variants from API: {:?}", play_info.variants);
...
pub async fn get_stream_url_with_quality(
    ...
    let douyu = DouYu::new(room_id)
        .await
        .map_err(|e| DtvError::internal(e.to_string()))?;
...
mod tests {
    ...
    fn variants_sample() -> Vec<DouyuRateVariant> {
        vec![
            DouyuRateVariant {
                name: "原画".to_string(),
                rate: 0,
    ...
    #[test]
    fn resolve_rate_origin_prefers_zero() {
        let variants = variants_sample();
        let rate = DouYu::resolve_rate_for_quality("原画", &variants);
        assert_eq!(rate, Some(0));
    }

    #[test]
    fn resolve_rate_high_prefers_rate_4() {
        let variants = variants_sample();
        let rate = DouYu::resolve_rate_for_quality("高清", &variants);
        assert_eq!(rate, Some(4));
    }

    #[test]
    fn resolve_rate_standard_prefers_rate_3() {
        let variants = variants_sample();
        let rate = DouYu::resolve_rate_for_quality("标清", &variants);
        assert_eq!(rate, Some(3));
    }

    #[test]
    fn resolve_rate_english_aliases() {
        let variants = variants_sample();
        assert_eq!(
            DouYu::resolve_rate_for_quality("origin", &variants),
            Some(0)
        );
        assert_eq!(DouYu::resolve_rate_for_quality("high", &variants), Some(4));
        assert_eq!(
            DouYu::resolve_rate_for_quality("standard", &variants),
            Some(3)
        );
    }
}
