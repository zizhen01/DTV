use crate::platforms::bilibili::state::{refresh_w_webid, BilibiliState};
...
pub async fn fetch_bilibili_live_list(
    ...
    let w_webid = match refresh_w_webid(state).await {
        Ok(id) => {
        ...
        Err(e) => {
            ...
            let fallback = state.w_webid.lock().ok().and_then(|g| g.clone());
            match fallback {
                Some(id) => {
                    println!(
