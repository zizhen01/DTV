...
export const usePlayerStore = defineStore("player", () => {
  ...
  const currentStreamer = computed(() => {
    ...
    return (
      activeStreamers.value.find(
        (s) => getPlayerKey(s.platform, s.roomId) === lastActiveKey.value,
      ) || null
    );
  });
  ...
  const isActive = computed(() => (platform: string, roomId: string) => {
    ...
    return activeStreamers.value.some(
      (s) => getPlayerKey(s.platform, s.roomId) === key,
    );
  });

  const _saveToStorage = () => {
    localStorage.setItem(
      "active_streamers_list",
      JSON.stringify(activeStreamers.value),
    );
  ...
  const setStreamerInfo = (info: StreamerInfoState) => {
    ...
    const index = activeStreamers.value.findIndex(
      (s) => getPlayerKey(s.platform, s.roomId) === key,
    );
  ...
  const updateStreamerInfo = (info: StreamerInfoState) => {
    const key = getPlayerKey(info.platform, info.roomId);
    const index = activeStreamers.value.findIndex(
      (s) => getPlayerKey(s.platform, s.roomId) === key,
    );
  ...
  const removeStreamer = (platform: string, roomId: string) => {
    ...
    if (lastActiveKey.value === key) {
      lastActiveKey.value =
        activeStreamers.value.length > 0
          ? getPlayerKey(
              activeStreamers.value[activeStreamers.value.length - 1].platform,
              activeStreamers.value[activeStreamers.value.length - 1].roomId,
            )
          : null;
    }
    _saveToStorage();
  ...
  const toggleMute = (platform: string, roomId: string) => {
    ...
    if (index !== -1) {
      activeStreamers.value[index].isMuted =
        !activeStreamers.value[index].isMuted;
      _saveToStorage();
    }
  };
