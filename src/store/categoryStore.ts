...
export const useCategoryStore = defineStore("category", () => {
  ...
  // Favorite Categories (Persistent)
  const favoriteCategories = ref<Array<{ id: string; title: string; platform: UiPlatform; type: string }>>(
    JSON.parse(localStorage.getItem("dtv_favorite_categories") || "[]")
  );

  const toggleFavoriteCategory = (item: { id: string; title: string; type: string }) => {
    const index = favoriteCategories.value.findIndex(
      (f) => f.id === item.id && f.platform === activePlatform.value
    );
    if (index > -1) {
    ...
    localStorage.setItem("dtv_favorite_categories", JSON.stringify(favoriteCategories.value));
  };

  const toggleFavoriteCategoryForPlatform = (
    platform: UiPlatform,
    item: { id: string; title: string; type: string },
  ) => {
    const index = favoriteCategories.value.findIndex(
      (f) => f.id === item.id && f.platform === platform,
    );
    if (index > -1) {
      favoriteCategories.value.splice(index, 1);
    } else {
    ...
    localStorage.setItem(
      "dtv_favorite_categories",
      JSON.stringify(favoriteCategories.value),
    );
  };

  const isFavorite = (id: string) => {
    return favoriteCategories.value.some(
      (f) => f.id === id && f.platform === activePlatform.value
    );
  };

  const categoriesData = computed(() => {
    if (activePlatform.value === "douyin")
      return douyinCategoriesData as Category1[];
    if (activePlatform.value === "huya")
      return huyaCategoriesData as Category1[];
    if (activePlatform.value === "bilibili")
      return biliCategoriesData as Category1[];
    return [] as Category1[];
  });
    ...
    if (isDouyu.value) {
      return douyuCate1List.value.map((c1: Category1) => ({
        ...
        items: douyuCate2List.value
          .filter(
            (c2: Category2) => (c2 as any).cate1Id === (c1.cate1Id || c1.id),
          )
          .map((c2: Category2) => ({
            id:
              (c2 as any).shortName ||
              (c2 as any).short_name ||
              String(c2.id || ""),
      ...
      return categoriesData.value.map((c1: Category1) => ({
        id: c1.href || "",
        title: c1.title || "未知分类",
        items: ((c1.subcategories || []) as Category2[]).map(
          (c2: Category2) => ({
            id: c2.href || "",
            title: c2.title || "未知分类",
          }),
      ...
      if (douyuCate2List.value.length > 0 && !selectedCategoryInfo.value) {
        ...
        if (firstC1) {
          const firstC2 = douyuCate2List.value.find(
            (c2: Category2) =>
              (c2 as any).cate1Id === (firstC1.cate1Id || firstC1.id),
          );
          if (firstC2) {
            selectedCategoryInfo.value = {
              type: "cate2",
              id:
                (firstC2 as any).shortName ||
                (firstC2 as any).short_name ||
                String(firstC2.id),
  ...
  const initCommonData = () => {
    if (categoriesData.value.length > 0 && !selectedCategory.value) {
      ...
      if (
        firstC1 &&
        firstC1.subcategories &&
        firstC1.subcategories.length > 0
      ) {
  ...
  const handleCategorySelect = (item: { id: string; title: string }) => {
    if (isDouyu.value) {
      const match = douyuCate2List.value.find(
        (c: Category2) =>
          ((c as any).shortName || (c as any).short_name) === item.id,
      );
      if (match) {
  ...
  return {
    ...
    toggleFavoriteCategoryForPlatform,
    isFavorite,
  };
});
