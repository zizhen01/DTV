  ...
  actions: {
    async initTheme() {
      ...
      if (storedTheme === "dark" || storedTheme === "light") {
        this.userPreference = storedTheme;
      }
      ...
      if (storedColor) {
        ...
        document.documentElement.setAttribute("data-theme", this.userPreference);
        try {
          const win = WebviewWindow.getCurrent();
          await win.setTheme(this.userPreference);
        } catch (error) {
          console.error(
            "[ThemeStore] Error setting Tauri window theme:",
            error,
          );
        }
      }
    ...
    toggleTheme() {
      ...
      localStorage.setItem(THEME_KEY, this.userPreference);
      this._applyTheme();
    },
    ...
    async _applyTheme() {
      const themeToApply = this.userPreference;
