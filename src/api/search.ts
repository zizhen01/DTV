...
export async function searchHuyaAnchors(
  keyword: string,
  page = 1,
): Promise<HuyaAnchorItem[]> {
...
export async function searchBilibiliRooms(
  keyword: string,
  page = 1,
): Promise<BilibiliSearchItem[]> {
  return invoke<BilibiliSearchItem[]>("search_bilibili_rooms", {
    keyword,
    page,
  });
}
