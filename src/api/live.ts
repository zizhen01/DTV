...
import type {
  GetLiveStreamRequestV2,
  LiveStreamResponseV2,
} from "../types/api/live";

export async function getLiveStreamV2(
  request: GetLiveStreamRequestV2,
): Promise<LiveStreamResponseV2> {
...
export async function fetchDouyuLiveList(
  cate2: string,
  offset: number,
  limit: number,
): Promise<any> {
...
export async function fetchDouyuLiveListForCate3(
  cate3Id: string,
  page: number,
  limit: number,
): Promise<any> {
...
export async function fetchDouyinPartitionRooms(
  ...
  return invoke("fetch_douyin_partition_rooms", {
    partition,
    partitionType,
    offset,
    msToken,
...
export async function fetchHuyaLiveList(
  iGid: string,
  iPageNo: number,
  iPageSize: number,
): Promise<any> {
...
export async function fetchBilibiliLiveList(
  areaId: string,
  parentAreaId: string,
  page: number,
): Promise<string> {
  return invoke<string>("fetch_bilibili_live_list", {
    areaId,
    parentAreaId,
    page,
  });
