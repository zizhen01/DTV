...
export function useCategories(
  ...
  const fetchCategories = async () => {
    try {
      // Use API wrapper
      const response =
        (await fetchDouyuCategories()) as RustFrontendCategoryResponse;
  ...
  const fetchThreeCate = async (cate2Id: number) => {
    ...
    try {
      const threeCategoriesResult = (await fetchDouyuThreeCate(
        cate2Id,
      )) as CommonPlatformCategory[];
  ...
  const sortedCate2List = computed(() => {
    ...
    const filtered = cate2List.value
      .filter(
        (cate2: Category2) => (cate2 as any).cate1Id === selectedCate1Id.value,
      )
      .sort(
        (a: Category2, b: Category2) =>
          ((b as any).count || 0) - ((a as any).count || 0),
      );
    return filtered;
  });
