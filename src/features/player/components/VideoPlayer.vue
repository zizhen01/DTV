  ...
  >
    <div class="player-layout">
      <div class="main-content">
        ...
        <div v-else class="player-container group relative">
...
import type { LiveStatus } from "../../../types/app/status";
import type {
  DanmakuMessage,
...
const props = defineProps<PlayerProps & { isActive?: boolean; isVisible?: boolean }>();
...
watch(
  ...
  (visible) => {
    if (visible) {
      if (danmuInstance.value && !danmuInstance.value.play) {
        // Resume danmu
        danmuInstance.value.play?.();
      }
    } else {
      if (danmuInstance.value) {
        // Pause danmu rendering to save CPU when hidden
...
const stopProxy = async () => {
  ...
  if (Object.keys(playerStore.activeStreamers).length <= 1) {
    ...
    console.log(
      "[Player] Skipping global proxy stop as other players are active",
    );
  }
};
...
async function initializePlayerAndStream(
  ...
  console.log(
    `[Player] Initializing player for ${pPlatform} room ${pRoomId}, refresh=${isRefresh}`,
  );
  ...
  try {
    ...
    console.log(
      `[Player] Retrieved stream URL for ${pPlatform} room ${pRoomId}:`,
      streamUrl,
    );
    if (!streamUrl) {
    ...
    await mountXgPlayer(streamUrl, pPlatform, pRoomId, streamType);
  } catch (error: any) {
    console.error(
...
watch(
  ...
  () => {
    if (props.roomId && props.platform) {
      // Preserve existing liveStatus from store if available, otherwise derive from boolean
      const existing = playerStore.activeStreamers.find(
        (s) => s.platform === props.platform && s.roomId === props.roomId
      );
      let status = existing?.liveStatus;
      if (!status) {
         status = playerIsLive.value ? "LIVE" : "OFFLINE";
      }
      
      // If we just detected offline error, force OFFLINE
      if (isOfflineError.value) {
        status = "OFFLINE";
      }

      playerStore.updateStreamerInfo({
        roomId: props.roomId,
        platform: props.platform,
        title: playerTitle.value ?? "",
        anchorName: playerAnchorName.value ?? "",
