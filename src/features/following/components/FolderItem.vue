<template>
  <motion.div
    class="relative mb-2 overflow-hidden rounded-[18px] border border-border-main/70 bg-surface-low/60 select-none"
  ...
  >
    <div
      class="group flex cursor-pointer items-center gap-3 rounded-[12px] bg-transparent px-3.5 py-2"
    ...
    >
      ...
      <template v-if="!collapsed">
        <span class="flex-1 truncate text-[13px] font-black tracking-[0.02em]">{{
          folder.name
        }}</span>
        <span class="rounded-full border border-border-main/70 bg-surface-high/60 px-2 text-[10px] font-extrabold">{{
    ...
    <AnimatePresence :initial="false">
      <motion.div
        ...
        class="relative overflow-hidden px-3 pt-2 pb-3 [will-change:height]"
        :class="{ 'pointer-events-none': globalDragging, 'px-1': collapsed }"
      ...
      >
        ...
        />
        <ul class="relative z-[1] flex flex-col gap-1.5">
          <li
            v-for="(streamer, index) in folderItems"
            :key="`${streamer.platform}:${streamer.id}`"
            class="rounded-[12px] border border-border-main/60 bg-surface-low/50 transition-colors hover:border-border-strong hover:bg-surface-high/70"
